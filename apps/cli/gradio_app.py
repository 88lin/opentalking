...
from opentalking.engine.accelerator import device_type, synchronize
...
def run_multi_gpu_inference(
    ...
    # CUDA_VISIBLE_DEVICES/ASCEND_RT_VISIBLE_DEVICES=... torchrun --nproc_per_node=... generate_video.py ...
    cmd = [
        "torchrun",
        f"--nproc_per_node={num_gpus}",
        "apps/cli/generate_video.py",
        "--ckpt_dir", ckpt_dir,
    ...
    if device_type() == "npu":
        ...
        env["ASCEND_VISIBLE_DEVICES"] = visible_devices
    else:
        env["CUDA_VISIBLE_DEVICES"] = visible_devices
    
    logger.info(f"Starting Multi-GPU inference with command: {' '.join(cmd)}")
    logger.info(f"Visible Devices: {visible_devices}")
