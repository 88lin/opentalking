...
from opentalking.engine.audio.loudness import RotaryPositionalEmbedding1D, normalize_and_scale, split_token_counts_and_frame_ids
...
class SingleStreamAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, encoder_hidden_states: torch.Tensor, shape=None, enable_sp=False, kv_seq=None, visual_seqlen=None) -> torch.Tensor:
...
class SingleStreamMutiAttention(SingleStreamAttention):
    ...
    def forward(self, 
        ...
        encoder_k = rearrange(encoder_k, "B H M K -> B M H K")
        encoder_v = rearrange(encoder_v, "B H M K -> B M H K")
        x = attention_ops.memory_efficient_attention(q, encoder_k, encoder_v, attn_bias=None, op=None,)
        x = rearrange(x, "B M H K -> B H M K")
